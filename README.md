...
DocGenAI supports flexible metadata generation with three configurable modes to suit different documentation workflows.

### Metadata Modes

#### **None** - No Metadata

Generate clean documentation without any metadata footer:

```bash
# Generate documentation without metadata
poetry run docgenai generate src/models.py --metadata-mode none
```

#### **Footer** (Default) - Inline Metadata

Include metadata as a footer section in the documentation file:

```bash
# Generate with metadata footer (default behavior)
poetry run docgenai generate src/models.py --metadata-mode footer
```

#### **File** - Separate Metadata File

Save metadata as a separate `.metadata.md` file alongside the documentation:

```bash
# Generate with separate metadata file
poetry run docgenai generate src/models.py --metadata-mode file
```
...
- **Files Analyzed**: Total count and processing details
- **Generation Details**: Timestamp, model used, processing time
- **File Tree**: Structure of analyzed files
- **Processing Info**: Chunking strategy, token limits
- **Platform Info**: Backend used (MLX/Transformers) and system details

### Configuration-Based Metadata

Set default metadata mode in `config.yaml`:
...
### Files Analyzed
- **Total files**: 1
- **Generation time**: 2025-07-03 17:45:42
...
**Main file**: `models_documentation.md` (clean documentation)
**Metadata file**: `models_documentation.metadata.md`
...
| Mode | Best For | Benefits |
|------|----------|----------|
| **None** | Clean docs, publishing | No metadata clutter |
| **Footer** | Development, review | Everything in one file |
| **File** | CI/CD, automation | Separate concerns, easy parsing |
...
poetry run docgenai generate src/models.py --metadata-mode none          # Clean docs without metadata
poetry run docgenai generate src/models.py --metadata-mode footer        # Metadata in footer (default)
poetry run docgenai generate src/models.py --metadata-mode file          # Separate metadata file

# Combine multi-file, chaining, project type, and metadata options
poetry run docgenai generate src/ --multi-file --chain --chain-strategy enhanced --project-type application --metadata-mode file
```
...
output:
  ...
  metadata_mode: "footer"      # Options: "none", "footer", "file"
```
